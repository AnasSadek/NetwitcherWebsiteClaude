...
export function HeadTurn({
  ...
  /** Vertikaler Kopf-Versatz in CSS-Pixeln (kleines „Nicken"). Muss klein
   *  bleiben (± ~12 px): innerhalb des Matte-Rands deckt der gezeichnete
   *  Frame den frontalen Poster-Kopf weiterhin vollständig ab. */
  ...
  const draw = () => {
    ...
    if (Math.abs(f - last.current.f) < 0.003 && Math.abs(oy - last.current.oy) < 0.1) return;
    ...
    ctx.globalAlpha = 1;
    ctx.drawImage(list[a]!, 0, dy, w, h);
    if (b >= 0 && b !== a && t > 0.001) {
      ctx.globalAlpha = t;
      ctx.drawImage(list[b]!, 0, dy, w, h);
      ctx.globalAlpha = 1;
    }
    // unvollständig gezeichnet → später erneut
    last.current = a === i0 && b === i1 ? { f, oy } : { f: NaN, oy };
  };
  const schedule = () => {
  ...
  useMotionValueEvent(offsetY ?? value, "change", () => {
    if (enabled && offsetY) schedule();
  });
