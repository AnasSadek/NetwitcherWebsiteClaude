...
export function ServiceCard({
  ...
  return (
    <Reveal delay={delay} className="h-full">
      <Link
        href={service.href}
        className={`group relative flex h-full flex-col overflow-hidden rounded border border-line bg-night-700/60 p-7 backdrop-blur transition-all duration-300 hover:-translate-y-1 ${accentBorder[service.color]}`}
      >
        <div
          aria-hidden="true"
        ...
        <div className="mb-5 flex items-center justify-between">
          <span
            className={`inline-flex h-13 w-13 items-center justify-center rounded border border-line bg-white/5 p-3 transition-transform duration-300 group-hover:scale-110 ${accentText[service.color]}`}
          >
        ...
        <ul className="mt-4 flex flex-wrap gap-1.5">
          {service.bullets.slice(0, 4).map((b) => (
            <li
              key={b}
              className="rounded border border-line px-2.5 py-1 text-[11px] text-mist"
            >
              {b}
            </li>
          ))}
