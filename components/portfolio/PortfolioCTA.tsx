 ...
 */
export function PortfolioCTA({ service, locale = "de" }: { service?: string; locale?: Locale }) {
  const t = getDict(locale);
  const href = service
    ? `${withLocale("/kontakt", locale)}?service=${encodeURIComponent(service)}`
      ...
      <div className="relative mx-auto max-w-4xl px-5 text-center sm:px-8">
        <Reveal>
          ...
          <BoxiTitle
            as="h2"
            id="portfolio-cta"
            lines={locale === "ar" ? ["أعجبك ما رأيت؟"] : ["GEFÄLLT DIR,", "WAS DU SIEHST?"]}
            max="4.5rem"
            min="2rem"
          ...
          <p className="mx-auto mt-6 max-w-md text-lg leading-relaxed text-ink-2">
            {locale === "ar" ? "لننجز معاً شيئاً يبقى في الأذهان." : "Dann lass uns etwas machen, das man sich merkt."}
          </p>
          <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
            ...
            >
              {t.nav.contactCta}
              <svg width="11" height="11" viewBox="0 0 100 100" aria-hidden="true" className="transition-transform duration-200 rtl:-scale-x-100 group-hover:translate-x-1 rtl:group-hover:-translate-x-1">
                <path d={ARROW_PATH} fill="currentColor" transform="rotate(90 50 50)" />
              </svg>
            </Link>
            <Link
              href={withLocale("/kontakt#termin", locale)}
          ...
          >
            {locale === "ar" ? "أو راسلنا مباشرة عبر واتساب" : "Oder direkt per WhatsApp schreiben"}
          </a>
        </Reveal>
      </div>
    </section>
